...
const char* logProperties = "log4j.rootLogger=DEBUG,A1,A4\n"
        "log4j.appender.A1=org.apache.log4j.RollingFileAppender\n"
        "log4j.appender.A1.appendFile=false\n"
        "log4j.appender.A1.file="
        "\nlog4j.appender.A1.layout=org.apache.log4j.PatternLayout\n"
        "log4j.appender.A1.layout.ConversionPattern=%-d{yyyy-MM-dd HH:mm:ss.zzz} [%t]  >> %m%n\n"
        "log4j.appender.A4=org.apache.log4j.ConsoleAppender\n"
        "log4j.appender.A4.layout=org.apache.log4j.PatternLayout\n"
        "log4j.appender.A4.layout.ConversionPattern=%-d{yyyy-MM-dd HH:mm:ss.zzz } [%t]  >> %m%n";
...
namespace Log4Qt
{
    ...
    bool PropertyConfigurator::configure(const QString path ,const QString &rConfigFilename)
    {
        QString configFilePath= path+"log4j.properties";
        PropertyConfigurator configurator;
        QString properties = QString(logProperties);
        QString logPath = QString("log4j.appender.A1.file=%1log/%2").arg(path).arg(rConfigFilename);
        qDebug("BBBBBBBBBBBBB %s", logPath.toUtf8().data());
        properties.replace("log4j.appender.A1.file=" , logPath);
        properties.replace("\\" , "/");
        QFile file(configFilePath);
        file.open(QFile::WriteOnly);
        file.write(properties.toLocal8Bit().data() , properties.size());
        file.close();
        return configurator.doConfigure(configFilePath);
    }
